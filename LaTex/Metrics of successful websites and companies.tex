...
\item \textbf{Pinterest} : As of October 2016, the content sharing service has reached 150 million monthly active users.\cite{key30}
...
\newpage
\subsection{Metrics}
Now that we have declared the companies that we are going to use we will also need to decide which metrics are we going to examine. Since we cannot have access to metrics that are available in the companies from tools such as Google analytics\cite{key36}, we will have to examine metrics that are more related to how the site is structure and what exactly does the home page of each site includes. Initially we can divide the metrics in two major categories:
\begin{itemize}
\item What we see?
\item What lays behind of what we see?
\end{itemize}
In the first category we are referring to metrics that can easily be conceived by the naked eye as well. For example the images that a website is using in its landing page. How many there are and if they are big or small. From the empirical studies that have taken place we can see that this variables have already been examined but not from the actual sizes that they have in a website but from the users perspective of what constitutes a visually pleasant website.\\
...
\bibitem{key27}Forbes (2016). Retrieved February 26, 2017, from $https://www.forbes.com/sites/kathleenchaykowski/2016/10/13/pinterest-reaches-150-million-monthly-users/$\#$2a5f63f4732e$
...
\bibitem{A3}Tables Generator (2017). Retrieved January 29, 2017, from $http://www.tablesgenerator.com/$

\bibitem{A4}Continuum (2017). Retrieved January 29, 2017, from $https://www.continuum.io/downloads$

\bibitem{A5}Validator W3 (2017). Retrieved January 29, 2017, from $https://validator.w3.org/$

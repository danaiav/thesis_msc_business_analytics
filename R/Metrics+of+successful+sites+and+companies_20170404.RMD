...
ggplot(data=total_500_final,aes(x=Assets))+geom_histogram(binwidth=50, colour = "red", fill ="darkred")
ggplot(data=total_500_final,aes(x=Market_Value))+geom_histogram(binwidth=50, colour = "blue", fill ="darkblue")
ggplot(data=total_500_final,aes(x=Total_SH_Equity))+geom_histogram(binwidth=50, colour = "purple", fill ="pink")
...
set.seed(220)
clusters <- hclust(dist(total_500_final[, 1]))
plot(clusters)
```


```{r}
fortuneCluster <- kmeans(total_500_final[, 1], 4, iter.max = 500,nstart = 1)
...
```{r}
ggplot(total_500_final, aes(.bmp, loading.time, color = fortuneCluster$cluster)) + geom_point(size=3)
```

...
```{r}
#Now we create a new model with only the variables with coef different from NA so as to have a cleare look
...
#We can see here that some of the variables are correlated also with each other so we can try keeping only the variables that are not correlated with each other so as to avoid multicollinearity. The variables that are correlated with each other are the image sizes that are included in the model so we will keep only one of those variables. The one that is correlated the most with all the others so as to keep the biggest part of the information.
```
...
#Here the image size that is devided in the best way is 15x12 so we will create a model that we will put only this size
```

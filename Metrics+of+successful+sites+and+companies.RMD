...
ggplot(total_500_final, aes(Market.value.., Ranking)) + geom_point()
ggplot(total_500_final, aes(Total.Stockholder.Equity.. , Ranking)) + geom_point()
...
full_4 <- lm(Revenues~ im_s_large+ im_s_medium+ im_s_small+ im_s_thumbnail,data=total_500_final_r)
summary(full_4)
...
aic_full4
aic_ma 
#Also the best AIC is for model a as well (the smaller the better)
###################################################################################################
#Clustering
#Based on those results we will try to cluster the companies based on the results of the regression
total_500_final_reg <- total_500_final_r[,c(1,3,4,6,10,11,18,19,21,22,23,24)]
names(total_500_final_reg)
set.seed(20)
fortuneCluster <- kmeans(total_500_final_reg[, 1:11], 5, nstart = 20)
